            ...
            TString myMethodList = "Cuts" )
{
   ...
   TMVA::Factory *factory = new TMVA::Factory( "tmvaD0", outputFile,
   ...
   factory->AddVariable("decayLength", 'F');
   factory->AddVariable("dca12", 'F');
   factory->AddVariable("dcaV0ToPv", 'F');
   //   factory->AddVariable("cosThetaStar", 'F');
   //   factory->AddVariable("cosTheta", 'F');
   factory->AddVariable("dcaKaon", 'F');
   factory->AddVariable("dcaPion", 'F');
   //   factory->AddVariable("ptKaon", 'F');
   //   factory->AddVariable("ptPion", 'F');
   ...
   factory->AddSpectator("m", 'F');

   // Read training and test data
   // (it is also possible to use ASCII format as input -> see TMVA Users Guide)
   TFile* inputSignal = new TFile(signaFileName.Data());
   TTree* signal     = (TTree*)inputSignal->Get("ntpD0");
   TH1F*  hMcPt      = (TH1F*)inputSignal->Get("hMcPt");
   
   // --- Register the training and test trees
   double const signalWeight = 1.0;
   factory->AddSignalTree(signal, signalWeight);

   TFile* inputBackground = new TFile(backgroundFileName);
   TTree* backgroundSameSign = (TTree*)inputBackground->Get(Form("ntpD0BackgroundSameSignPt%i%i",(int)PtBins[ptBin],(int)PtBins[ptBin+1]));
   // TTree* backgroundSideBand = (TTree*)inputBackground->Get(Form("ntpD0BackgroundSideBandPt%i%i",(int)PtBins[ptBin],(int)PtBins[ptBin+1]));

   double const backgroundWeight = 1.0;
   ...
   int const nOriginalSignalEntries = hMcPt->Integral(hMcPt->FindBin(PtBins[ptBin]),hMcPt->FindBin(PtBins[ptBin+1])); // Number of simulated D0/D0bar in this pT bin before efficiency
    TString signalWeightExpression = TString::Format("matchHftWeight*ncoll*w*((%f/%f)*0.8*2.*3.14*pt*2*(%f)*2.*exp(-1.45-1.73*pt)*0.0389)",
                                                     (float)totalNumberOfEvents, (float)nOriginalSignalEntries, tmvaCuts::PtBins[ptBin+1]-tmvaCuts::PtBins[ptBin]);
   ...
   TCut mycuts = Form("pt>%1.2f && pt<%1.2f && ptKaon>%1.2f && ptPion>%1.2f && "
                      "decayLength>%f && decayLength<0.2 && "
                      "dca12<%f && "
                      "dcaKaon>%f && dcaKaon<0.2 && "
                      "dcaPion>%f && dcaPion<0.2", 
                      PtBins[ptBin], PtBins[ptBin+1], minPt, minPt, 
                      tmvaCuts::decayLength[ptBin], tmvaCuts::dcaDaughters[ptBin],
                      tmvaCuts::kDca[ptBin], tmvaCuts::pDca[ptBin]);
   ...
   signal->Draw("pt>>hPtSignal", signalWeightExpression*mycuts, "e");
   backgroundSameSign->Draw("pt>>hPtBackgroundSameSign", backgroundWeightExpression*mycuts, "e");
   // backgroundSideBand->Draw("pt>>hPtBackgroundSideBand", backgroundWeightExpression*mycuts, "e");
   ...
   factory->PrepareTrainingAndTestTree( mycuts, nSignal/2, nBackground/2, nSignal/2, nBackground/2);
   ...
   if (Use["Cuts"])
      factory->BookMethod( TMVA::Types::kCuts, "Cuts",
                           "!H:!V:FitMethod=MC:EffSel:SampleSize=20000000:VarProp=FSmart" );
...
void printInitialCuts(int const ptBin)
{
   ...
   cout << "decayLength  > " << tmvaCuts::decayLength[ptBin] << "\n";
   cout << "CosThata     > " << tmvaCuts::cosTheta[ptBin] << "\n";
   cout << "kaonDca      > " << tmvaCuts::kDca[ptBin] << "\n";
   cout << "pionDca      > " << tmvaCuts::pDca[ptBin] << "\n";
   cout << "------------------------------------------------------------------\n";
}
