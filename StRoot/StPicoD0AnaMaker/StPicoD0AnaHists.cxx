...
StPicoD0AnaHists::StPicoD0AnaHists(TString fileBaseName) : mPrescales(NULL), mOutFile(NULL), 
  ...
  mh1Cent(NULL), mh1gRefmultCor(NULL), mh1gRefmultCorWg(NULL), mh3InvariantMassVsPtVsCent(NULL), mh3InvariantMassVsPtVsCentLike(NULL), mh3InvariantMassVsPtVsCentTof(NULL), mh3InvariantMassVsPtVsCentTofLike(NULL),
...
//---------------------------------------------------------------------
void StPicoD0AnaHists::addKaonPion(StKaonPion const* const kp, bool unlike, bool tpc, bool tof, int centrality)
{
  if(unlike)
  {
    if(tpc) mh2InvariantMassVsPt->Fill(kp->pt(),kp->m());
    if(tof) mh2InvariantMassVsPtTof->Fill(kp->pt(),kp->m());
    if(tpc) mh3InvariantMassVsPtVsCent->Fill(kp->pt(),centrality,kp->m());
    if(tof) mh3InvariantMassVsPtVsCentTof->Fill(kp->pt(),centrality,kp->m());
  }
  else
  {
    if(tpc) mh2InvariantMassVsPtLike->Fill(kp->pt(),kp->m());
    if(tof) mh2InvariantMassVsPtTofLike->Fill(kp->pt(),kp->m());
    if(tpc) mh3InvariantMassVsPtVsCentLike->Fill(kp->pt(),centrality,kp->m());
    if(tof) mh3InvariantMassVsPtVsCentTofLike->Fill(kp->pt(),centrality,kp->m());
  }
}
//---------------------------------------------------------------------
