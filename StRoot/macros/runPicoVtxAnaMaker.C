...
StChain *chain;
void runPicoVtxAnaMaker(const Char_t *inputFile, const Char_t *outputFile="test.root")
{ 
  ...
  {
      cout<<"Environment Star Library does not match the requested library in runPicoVtxAnaMaker.C. Exiting..."<<endl;
      exit(1);
  }
  ...
  gSystem->Load("StPicoVtxAnaMaker");

	chain = new StChain();

	StPicoDstMaker* picoDstMaker = new StPicoDstMaker(0,inputFile,"picoDstMaker");
  StPicoVtxAnaMaker* picoVtxAnaMaker = new StPicoVtxAnaMaker("picoVtxAnaMaker",picoDstMaker,outputFile);
